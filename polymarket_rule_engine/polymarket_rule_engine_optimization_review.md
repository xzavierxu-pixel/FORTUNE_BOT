...
## 0. 实施状态更新（截至 2026-03-07）

下面这一节只跟踪“已经工程化进入代码主干”的需求，不把纯研究建议误记为工程缺陷。

### 0.1 已完成

1. `offline / online` 产物路径拆分已落地，研究产物写入 `data/offline/`，避免再与历史全样本产物混用。
2. `train / valid / test` 三段式切分已落地，并在规则训练、模型训练、分析、回测中统一使用。
3. 规则统计的未来信息污染已修复：离线规则估计不再由全样本直接回填，离线规则筛选只使用 `train + valid` 口径。
4. 回测期与规则筛选期已分离：离线回测只使用 `test` 集。
5. `earliest-only` 已落地：每个 `market_id` 在离线回测中只保留最早的一次候选交易。
6. 严格 OOS 报告已落地：模型训练摘要、规则训练摘要、回测摘要都输出到 `data/offline/metadata/`。
7. 小数据集调试入口已提供：训练、回测和统一 workflow 支持 `--max-rows`、`--recent-days`。
8. 分析脚本已切换到离线产物：`analyze_q_model_calibration.py`、`analyze_alpha_quadrant.py`、`analyze_rules_alpha_quadrant.py` 默认分析离线产物。
9. 模型训练里原先把 `e_sample = y - price` 这样的结果泄漏特征带进模型的问题已修复。
10. `q-only` / `tradeable-only` / `two-stage` / `residual_q` baseline 家族的统一离线对比与 walk-forward 多窗口验证已实现，结果输出到 `data/offline/analysis/baseline_family_*.csv` 和 `baseline_family_walk_forward_*.csv`。
11. 主流程训练目标已扩展为可切换的 `q` / `residual_q` / `expected_pnl` / `expected_roi`，不再只停留在研究层 baseline。
12. 分组校准已进入主流程，当前支持全局、按 `domain`、按 `horizon` 的验证集校准模式。
13. 规则先验增强已落地：经验贝叶斯收缩、Wilson 下界过滤、BH-FDR 多重检验已经接入规则训练流程。
14. 原始数据与 snapshot 源头治理已落地到代码：包括 outcome token / outcome 顺序显式校验、raw/snapshot quarantine 输出、snapshot 命中率与缺失模式统计、stale quote 质量标记、以及更严格的 raw market 过滤。
15. 特征工程已补入主流程：价格路径 / term structure、盘口与流动性代理特征、轻量文本 embedding、事件结构特征、时间质量特征都已进入可训练特征集。
16. 回测风控已增强：domain/category/cluster/settlement/side 限仓、容量约束、domain whitelist、以及基于统计量的 rule kill-switch 已接入。
17. 统一 workflow 已补齐：新增了离线统一入口 `rule_baseline/workflow/run_pipeline.py` 和在线入口 `rule_baseline/workflow/run_online_pipeline.py`。

### 0.2 部分完成

1. `online` 流程已从“仅有目录”升级为可执行 workflow，但仍未形成真正的定时调度、监控告警、发布审计和回滚体系。
2. walk-forward 与 baseline 家族已经能通过统一 workflow 触发，但主训练脚本和主回测脚本本身仍然默认是单窗口入口，没有完全改造成“所有模式同一主命令”。
3. 组合风险控制已从“几乎没有”升级为簇限仓、容量上限和动态风险预算，但仍不是显式的相关性矩阵建模，更不是跨市场净敞口优化。
4. domain-specific 适配已实现到 calibration 层，但还没有进一步发展成 domain specialists / mixture-of-experts。
5. 源头治理代码已落地，但历史产物如果没有重新执行 `fetch_raw_events.py` / `build_snapshots.py`，旧文件不会自动带上新的 quarantine、audit 和质量标记字段。

### 0.3 未完成

1. 跨市场对冲、事件级相关性建模、显式相关矩阵或协方差驱动的组合优化仍未实现。
2. 提前平仓 / exit 策略仍未实现，当前主回测口径仍是建仓后持有到结算。
3. 事件驱动持仓生命周期回测仍未实现，当前还不是“同一市场后续 snapshot 更新已有仓位”的真实持仓框架。
4. 更深的微观结构特征仍未实现，例如真实订单簿深度、quote update rate、成交速率、部分成交与冲击成本模型。
5. 更强的文本语义建模仍未实现，当前只是轻量本地 embedding / 结构特征，还没有接入专门的句向量模型或外部语义模型。
6. regime 检测、specialists、自动降仓 / 停用策略切换仍未实现。
7. 完整的线上生产化体系仍未实现，包括自动调度、健康检查、发布验收、回滚和线上监控。

### 0.4 当前实现后的最新离线结论

在严格离线框架下，旧回测中那种极端收益已经消失。当前全量离线验证的结果是：

1. `test` 集上，Q-Model 并没有优于市场 price：
   - `price AUC = 0.5099`
   - `q_pred AUC = 0.4996`
   - `price LogLoss = 0.6898`
   - `q_pred LogLoss = 0.6918`
2. earliest-only 严格 OOS 回测结果为负：
   - `183` 笔交易
   - `ROI = -14.85%`
   - `Max Drawdown = -18.66%`
3. baseline 对比也已经有了第一版离线结论：
   - `tradeable-only` 在 `test` 上的 `precision@50 = 1.00`，高于 `q-only` 的 `0.46`
   - 但三套 baseline 的 earliest-only 扁平回测都仍为负收益
   - 当前 `tradeable-only` 的离线扁平回测三者中最好，但仍只有 `ROI = -12.48%`
4. walk-forward 三窗口结果也已支持同样结论：
   - 四套 baseline（`q-only`、`residual_q`、`tradeable-only`、`two-stage`）都出现明显窗口不稳定性
   - `wf_1` 为正收益，但 `wf_2`、`wf_3` 明显为负，说明当前 alpha 不稳
   - 三窗口平均 ROI 仍全部为负，其中 `residual_q` 与 `tradeable-only` 最接近持平，但仍分别只有 `-10.19%` 与 `-10.21%`
5. 因此，文档中“先修研究可信度，再谈放大利润”的优先级判断，已经被最新实现结果验证。
...
这个问题需要分成 **offline** 和 **online** 两个场景看待。

1. 对 **offline 研究/回测** 来说，全样本规则一定会污染评估，必须禁止。
2. 对 **online 实时预测/最新部署** 来说，在最终上线产物里使用全量历史数据重训规则和模型是合理的，目标本来就是尽量利用全部可用历史信息。
3. 因此正确做法不是“永远不能用全样本”，而是把研究产物和上线产物彻底拆开，不能混用同一套 artifacts。

当前确认的方向:

- `data/offline/`：严格数据隔离，只用于研究、回测、分析
- `data/online/`：允许用全量历史重训，只用于线上规则、模型、信号产出
- offline 结论绝不能引用 online 产物
- online 产物也不能回流去证明 offline 收益
...
这一项暂时 **不作为第一阶段改造重点**。原因不是它不重要，而是当前更大的失真主要来自数据隔离、规则污染和重复下注。短期内应先优先解决:

1. offline / online 拆分
2. earliest-only 去重
3. 离线评估口径统一

等离线结果可信之后，再决定是否投入精力做组合相关性建模。
...
第一阶段不做复杂持仓建模，直接采用一个简单且强约束的口径:

- 每个 `market_id` 只保留 **最早的那一次** 交易机会
- 这里的“最早”定义为最小 `snapshot_time`
- 如果同一个最早时点有多个候选规则，只保留评分最高的一条

这样做的优点:

1. 实现简单，能立刻消除大量重复下注导致的虚假复利
2. 训练、离线回测、线上信号都可以统一口径
3. 比“同日只保留一条”更严格，也更容易解释

这个方案不是最终最优交易框架，但非常适合作为第一版可信评估基线。
...
**当前模型在优化“事件会不会发生”，但交易真正关心的是“这笔机会值不值得做，以及值多少钱”。**

这也是为什么后续建议里会强调，不要只看 `q_pred`，而要离线比较面向交易目标的 baseline。
...
- 继续预测事件概率 `q`
- 再用 `q - p` 或由 `q` 推出的 expected value 做交易决策
...
- 训练目标仍是全样本概率拟合，不是交易价值最优
- 容易把模型容量浪费在大量“不交易样本”上
- 对阈值附近样本不够敏感

### 路线 B: `tradeable-only`

- 不直接预测 `q`
- 直接预测一笔候选交易是否有正的 `signed_edge`
- 标签可定义为 `1[signed_edge > 0]`

其中:

- YES 方向: `signed_edge = y - price`
- NO 方向: `signed_edge = price - y`
...
### 路线 C: 双目标 / 双阶段 `two-stage`

第一阶段:

- 预测 `signed_edge > 0` 或 `signed_edge > threshold`
- 目标是先判断这笔机会是否值得进入候选池

第二阶段:

- 只在第一阶段通过的样本上，预测 `signed_edge` 大小
- 也可以改为预测 `expected pnl`、`expected roi` 或分桶后的 edge 等级

优点:

1. 第一阶段专门解决“是否有 edge”，减少 false positive
2. 第二阶段专门解决“edge 有多大”，更适合排序、打分和定仓
3. 比单纯预测 `q` 更贴近当前执行逻辑，因为当前执行本来就依赖规则方向和阈值过滤

为什么它可能比单纯预测 `q` 更好:

1. 如果 `q` 是完美真概率，那确实几乎没区别；但真实项目里的 `q_pred` 只是近似值，不是为交易目标训练出来的最优统计量。
2. `q-model` 会在全体样本上优化 logloss / Brier，而真实交易只关心少数会下单的样本。
3. 很多对 `q` 来说很小的误差，在交易上会变成“做/不做”的巨大差异，双阶段第一层更能捕捉这种边界问题。
4. 同样的 `q - p` 在不同价格区间的经济价值不同，第二阶段可以直接拟合 `signed_edge` 或 ROI，更贴近钱的尺度。

因此，当前最合理的看法不是“q-model 无用”，而是:

**`q-model` 更像世界模型，双阶段更像交易模型。**

后续不应直接主观判断哪条路线更优，而应在严格 offline 框架下做 baseline 对比。

---

## 4.8 当前确认的处理原则

结合本轮讨论，当前文档中的建议需要收敛到一个更明确的第一阶段改造边界。
...
1. `offline` 只服务于研究和评估
2. `online` 只服务于上线和最新信号生成
3. 两边都要记录 artifact version、训练截止时间、使用窗口
...
1. strictly offline train / valid / test
2. earliest-only 每个市场只取最早一次
3. 统一交易方向定义
4. 统一 top-k / threshold 选样规则

---

## 4.9 第八大问题: 数据采集与时间异常尚未被治理
...
### 5. 同一 `market_id` 只保留最早一次交易机会

建议:

- 每个 `market_id` 只保留最小 `snapshot_time`
- 如果同一最早时点出现多条规则，只保留评分最高的一条
- 训练、回测、线上信号统一采用这套 earliest-only 口径
...
这一项目前定义为 **low priority**。在第一阶段实施时可以先不做，理由是:

1. 当前更大的失真来自数据污染、样本隔离不严和重复下注
2. 在 offline 研究框架未跑通之前，先补复杂成本模型的收益不高
3. 应当等 earliest-only、offline / online 拆分、baseline 对比完成后，再决定是否投入

因此，第一阶段可以先在“无真实成本模型”的统一口径下完成调试和 baseline 比较。
...
其中最推荐优先验证的是 `two-stage`，但前提不是拍脑袋切换，而是在 offline 中和 `q-only` 做严格并排比较。
...
### 11.1 为什么不能只主观看 `q_pred`

即便在暂时不考虑手续费和相关性的前提下，`q_pred` 也不等于最好的交易分数。原因有三点:

1. `q-model` 优化的是全样本概率拟合，而交易只关心少数会被下单的样本。
2. 对 `q` 来说很小的误差，在阈值决策里会变成“做/不做”的离散跳变。
3. 同样的 `q - p` 放在不同价格区间，经济价值并不相同。

所以后续更应关注:

- `signed_edge`
- `positive-edge precision`
- top-k 排序质量
- earliest-only 口径下的真实离线收益
...
## 7. 离线验证框架与三套 Baseline

这一节用于回答一个关键问题:

**到底是继续做 `q-only`，还是切到 `tradeable-only` / `two-stage`？**

正确做法不是先选答案，而是在统一 offline 框架下离线比较。
...
1. 使用 `data/offline/`
2. 严格 train / valid / test 切分
3. 规则先验只由 train 产生
4. earliest-only：每个 `market_id` 只取最早一次
5. 相同的候选池、相同的 top-k 规则、相同的评估窗口
6. 开发调试阶段优先使用小数据集，而不是默认全量历史

## 7.1.1 小数据集调试原则

当前原始数据从 `2024-11-01` 开始累积，全量流程在调试阶段过重，反馈周期太长。实施时应明确分成两步:

第一步: 小数据集调试

- 只截取一小段时间窗口
- 只保留少量 domain / market_type / market_id
- 目标是快速验证:
  - offline / online 路径是否正确
  - earliest-only 逻辑是否正确
  - baseline 训练和评估脚本是否跑通
  - 产物命名和版本管理是否一致
...
也就是说，小数据集的目标是 **调试正确性和迭代速度**，不是替代最终评估。

## 7.2 Baseline A: `q-only`
...
## 7.3 Baseline B: `tradeable-only`

定义:

- 直接预测 `1[signed_edge > 0]`
- 或更严格一些，预测 `1[signed_edge > threshold]`

其中:

- YES: `signed_edge = y - price`
- NO: `signed_edge = price - y`
...
- 测试“直接做交易分类”是否比“先预测 q 再交易”更适合过滤假信号

## 7.4 Baseline C: `two-stage`

定义:

第一阶段:

- 预测 `1[signed_edge > 0]` 或 `1[signed_edge > threshold]`
...
- 测试把“过滤”和“排序”拆开后，能否提高 top-k 质量

## 7.5 推荐的比较指标

不要只比 AUC / LogLoss，建议统一输出:

- valid / test 上的 `mean signed_edge`
- top-k `mean signed_edge`
- `precision@k`：top-k 中 `signed_edge > 0` 的比例
- `recall@k`：抓到的正 edge 样本覆盖率
- 分位数组收益曲线
- earliest-only 口径下的回测收益
- 各 domain / market_type / horizon 的稳定性

## 7.6 推荐的判断标准

如果出现以下情况，说明 `two-stage` 值得继续:

1. `precision@k` 明显高于 `q-only`
2. top-k `signed_edge` 更高
3. earliest-only 回测更稳，不只是收益更高
4. 不同 domain / horizon 上更稳定

如果 `two-stage` 只是让分类指标变漂亮，但 top-k 交易质量没有提升，就不值得切换。
...
- 这一步默认先在小数据集上跑通
- 确认路径、标签、earliest-only、baseline 逻辑都正确后
- 再切回全量历史数据执行正式评估

## 8.2 第二阶段: 先压回撤，再谈放大利润
