...
class GroupKeyServingAssetsTest(unittest.TestCase):
    def test_group_defaults_are_aggregated_from_group_key_rows(self) -> None:
        ...
        self.assertNotIn("group_unique_markets", group.columns)
        self.assertNotIn("group_snapshot_rows", group.columns)
        self.assertNotIn("group_market_share_global", group.columns)
        self.assertNotIn("group_snapshot_share_global", group.columns)
        self.assertNotIn("group_median_logloss", group.columns)
        self.assertNotIn("group_median_brier", group.columns)
        self.assertNotIn("global_group_logloss_q25", group.columns)
        self.assertNotIn("global_group_brier_q25", group.columns)
        self.assertNotIn("group_decision", group.columns)
        self.assertNotIn("full_group_expanding_logloss_tail_x_market_share", group.columns)
