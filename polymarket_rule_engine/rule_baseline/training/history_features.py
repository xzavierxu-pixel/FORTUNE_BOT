...
HISTORY_WINDOWS = {"expanding": None, "recent_50": 50, "recent_200": 200}
LEVEL_DEFINITIONS = {
    "global": [],
    "domain": ["domain"],
    "category": ["category"],
...
def history_sort_columns(frame: pd.DataFrame) -> list[str]:
    candidate_columns = ["closedTime", "snapshot_time", "snapshot_target_ts"]
    return [column for column in candidate_columns if column in frame.columns]


def windowed_history_slice(frame: pd.DataFrame, window_size: int | None) -> pd.DataFrame:
    if frame.empty:
        return frame.copy()
    ...
    if window_size is None:
        return ordered
    return ordered.tail(window_size).copy()
...
def summarize_history_features(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    for level_name, level_columns in LEVEL_DEFINITIONS.items():
        ...
        for window_name, window_size in HISTORY_WINDOWS.items():
            window_frame = windowed_history_slice(level_frame, window_size)
            grouped = (
            ...
            grouped = grouped.rename(
                columns={
                    ...
                    "abs_bias_max": f"{level_name}_{window_name}_abs_bias_max",
                    ...
                    "brier_max": f"{level_name}_{window_name}_brier_max",
                    ...
                    "logloss_max": f"{level_name}_{window_name}_logloss_max",
                }
            )
